...
degree= -20 # how many degrees are rotate by, please use 45 or 135 according to subject
rotation_degree=(degree/180.)*np.pi
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if expInfo['Flip X'] == (True):
            ...
            rotate_x.append((joy_x[-1]*math.cos(rotation_degree))-(joy_y[-1]*math.sin(rotation_degree)))
            rotate_y.append((joy_x[-1]*math.sin(rotation_degree))+(joy_y[-1]*math.cos(rotation_degree)))
        ...
        if t >= 2.0 and joy_circle.status == STARTED:  # only update if started and not stopped!
            ...
            if t >= 2.0 and (target_status==0):
               ...
               if ( (rotate_x[-1]<=0.1) and (rotate_x[-1]>=-0.1) and (rotate_y[-1]>=-0.1) and (rotate_y[-1]<=0.1)):
               ...
               if ( (rotate_x[-1]<=((targets[(target_number[-1])][0])+0.1) and (rotate_x[-1]>=((targets[(target_number[-1])][0])-0.1) 
               and (rotate_y[-1]>=((targets[(target_number[-1])][1])-0.1)) and (rotate_y[-1]<=((targets[(target_number[-1])][1])+0.1))))):
                target_circle.pos=[0,0]
                # Draw target cirlce in middle
                target_circle.tStart = t  # underestimates by a little under one frame
                target_circle.frameNStart = frameN
